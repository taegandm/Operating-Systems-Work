...
public class Priority implements Algorithm {
    ...
    public Task pickNextTask() {
    	//finds the highest priority in the list and returns it while removing it from the list
    	int highestPri = tasks.get(0).getPriority();
    	int index = 0;
    	
    	for(int i = 1; i < tasks.size(); i++) {
    		int currentPri = tasks.get(i).getPriority();
    		if(highestPri < currentPri) {
    			highestPri = currentPri;
    			index = i;
    		}
    	}
